...
The `home-routes` handler in the `y-video-postgres-swagger.routes.home` namespace
...
The `home-page` function will in turn call the `y-video-postgres-swagger.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `y-video-postgres-swagger.handler` namespace:
...
Request middleware functions are located under the `y-video-postgres-swagger.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `y-video-postgres-swagger.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `y-video-postgres-swagger.db.core` in some other namespace.
